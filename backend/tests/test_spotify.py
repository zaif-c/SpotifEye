...
async def test_spotify_service_top_artists():
    ...
    with patch("spotipy.Spotify") as mock_spotify:
        mock_spotify.return_value.current_user_top_artists.return_value = (
            MOCK_TOP_ARTISTS
        )
...
async def test_spotify_service_recently_played():
    ...
    with patch("spotipy.Spotify") as mock_spotify:
        mock_spotify.return_value.current_user_recently_played.return_value = (
            MOCK_RECENTLY_PLAYED
        )
        ...
        mock_spotify.return_value.current_user_recently_played.assert_called_once_with(
            limit=2
        )
